  ...
  (let [build-output (run-with-dir
    ...
    (if (seq (:err build-output))
      ...
      (let [apk-path (build-output->apk-name (:out build-output))
            ...
            test-build-command (format "ant -e package -Dtested.package_name=\"%s\" -Dtested.main_activity=\"%s\" -Dtested.project.apk=\"%s\" -Dandroid.api.level=\"13\" -Dkey.store=\"/Users/kapil/.android/debug.keystore\" -Dkey.store.password=\"android\" -Dkey.alias=\"androiddebugkey\" -Dkey.alias.password=\"android\""
